...
/// KOMODO_LASTMINED and prevKOMODO_LASTMINED apparently both are used to track which block height this notary node created.
/// if chain reorganises below KOMODO_LASTMINED it is rolled back to prevKOMODO_LASTMINED (see komodo_event_rewind())
/// KOMODO_LASTMINED is used to determine if the current notary node can make the next easy mining block (see BitcoinMiner())
int32_t KOMODO_LASTMINED,prevKOMODO_LASTMINED;

int32_t KOMODO_INSYNC,KOMODO_CCACTIVATE,JUMBLR_PAUSE = 1;
